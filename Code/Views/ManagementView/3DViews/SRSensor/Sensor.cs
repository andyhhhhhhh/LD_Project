...
namespace Smartray.Sample
{
    internal class Sensor : IDisposable
    {
        ...
        private UInt32 _numberOfProfilesToCapture = 0;

        // packet size
        private UInt32 _packetSize = 0;

        // true when live image acquisition is configured
        private bool _liveAcquisition = false;

        //true when MSR mode is enabled
        private bool _msrMode = false;

        // !!! use _currentImageData & _completedImageDatas instead received sensor image data
        private List<SensorImageData> _imageDatas = new List<SensorImageData>();

        //recieved point cloud data
        private List<PointCloud> _cbPointCloud = new List<PointCloud>();       // Point Clouds received from sensor via the callback function.

        private uint _numberOfCapturedProfiles;                     // Total number of profiles captured in the currect aquisition.

        private string _pointCloudFilePrefix;                                  // Filename-prefix, to store Point CLoud data.

        private float _transportResolution;                                    // x-axis resolution (transport resolution) for point cloud data.

        private bool _saveAllPoints;                                           // true: all points of the Point Cloud are stored in the file
                                                                               // false: only the
                                                                               //        valid points
                                                                               //        of the Point
                                                                               //        Cloud are
        ...
        private AutoResetEvent _imageCompleted = new AutoResetEvent(false);
        ...
        public delegate void ShowSensorLog(string strLog);
        public static ShowSensorLog m_ShowSensorLog;
        ...
        private static void Trace(string text)
        {
            ...
            if(m_ShowSensorLog != null)
            { 
                m_ShowSensorLog(text);
            }
        }

        // callback for data with not registered command numbers
        private static int UnknownSensorCommandCallback(Api.Sensor sensorObject)
        {
        ...
        public void Connect()
        {
            ...
            Api.SetSmartXccelerate(_sensorObject, true);
        }
        ...
        public void AddPointCloud(int numPoints,
                                ...
                                uint[] columnIdx)
        {
            PointCloud pointcloud = new PointCloud((uint)numPoints, point_cloud, intensity, laserlinethickness, profileIdx, columnIdx);
        ...
        {
            int acquisitionTimeoutMs = 1500000;
            ...
            while (wait)
            {
                ...
                if (_numberOfCapturedProfiles >= _numberOfProfilesToCapture)
                {
                    wait = false;
                }
            }
        ...
        public void WaitForImage(int totalExpectedImages)
        {
            ...
            //=======================================
            int acquisitionTimeoutMs = 150000;
            int acquisitionTimeoutStepMs = 100;
            while (true)
            {
        ...
        public void ConfigureDataTriggerToFrequencyHz(int triggerFrequencyHz = 10)
        {
            ...
            // Note: The internal data trigger frequency cannot be greater than the maximum scan rate
            //       achievable by the sensor in "Data Trigger Mode: Free Run", for the configured
            // ROI and Exposure Time)
