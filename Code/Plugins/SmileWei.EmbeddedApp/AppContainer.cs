    ...
    public partial class AppContainer : System.Windows.Forms.Panel
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                string strAppName = Path.GetFileNameWithoutExtension(this.AppFilename);

                m_AppProcess = Process.Start(info);
                while (true)
                {
                    var proInfo = Process.GetProcessesByName(strAppName);
                    if (proInfo.Length > 0)
                    {
                        //if (proInfo[0].MainWindowTitle.Contains(""))
                        //{
                        //    break;
                        //}
                        break;
        ...
        void Application_Idle(object sender, EventArgs e)
        {
            ...
            //ShowWindow(m_AppProcess.MainWindowHandle, SW_MINIMIZE);
